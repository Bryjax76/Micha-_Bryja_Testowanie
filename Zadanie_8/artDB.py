...
class dataBaseClass():     
    ...
    def update_all_kluby(liczba_pilkarzy, trener):

        conn = sqlite3.connect("mydatabase.db") 
        cursor = conn.cursor() 
        sql = "UPDATE KLUBY SET liczba_pilkarzy=? WHERE trener=?" 
        cursor.execute(sql, [(liczba_pilkarzy),(trener)]) 
        result = cursor.fetchall() 
        cursor.close() 
        conn.close() 
        return result 
...
if __name__ == '__main__': 
    if not os.path.exists("mydatabase.db"): 
        create_database() 
    print(select_all_kluby('AC Milan')) 
